...
- **`cn` 是国家后缀兜底**：由于裸 `cn` 已覆盖任意层级的 `.cn`（`baidu.cn`、`people.com.cn`、`x.y.cn` 等），**无需、也不应**再逐条添加 `360.cn`/`gov.cn`/`sina.com.cn` 等 `.cn` 明细，或 `*.gov.cn` 等通配——它们全被 `cn` 覆盖，纯属冗余（已实测验证删除后 `.cn` 仍直连 + 回退本地）。
- 注意区分：`.com` 国内服务（如 `cnblogs.com`）**不被 `cn` 覆盖**，仍需按"主页 + `*.主页`"显式添加。
...
- 搜索引擎：`google.com`、`duckduckgo.com`、`bing.com`（国际站走隧道）
- `cn.bing.com`（必应国内站）已加入直连列表；`bing.com` 国际站默认走隧道
